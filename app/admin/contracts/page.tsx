    ...
    <div className="space-y-10">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-400">
            Skapa och hantera avtalsprodukter för spot, portfölj och fastpris.
      ...
      <div className="rounded-3xl border border-gray-800 bg-gray-950 p-6">
        ...
        <div className="mt-4 text-xs text-gray-500">
          När avtalet skapas får det automatiskt en{' '}
          <span className="text-gray-200">Draft</span>-version i prishanteringen.
          Du behöver fylla priser per område och publicera för att avtalet ska gå
          LIVE.
        </div>
      </div>
              ...
              {contracts.map((contract) => {
                ...
                return (
                  <tr key={contract.id} className="border-t border-gray-800">
                    <td className="p-4 text-gray-200">{contract.name}</td>
                    <td className="p-4 font-mono text-xs text-gray-400">
                      {contract.slug}
                    </td>
                    <td className="p-4 text-gray-300">{contract.contract_type}</td>
                      ...
                      {state === 'LIVE' && (
                        <div className="text-emerald-300">
                          ...
                          <div className="mt-1 text-xs text-gray-500">
                            Gäller fr.o.m.{' '}
                            {published ? fmtDate(published.valid_from) : '—'}
                          </div>
                        </div>
                      )}
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <form action={setContractActive}>
                          ...
                          <button className="rounded-xl border border-gray-800 bg-black/40 px-3 py-2 text-xs text-gray-200 hover:border-cyan-500/40">
                            {contract.is_active ? 'Inaktivera' : 'Aktivera'}
                          </button>
                        </form>
