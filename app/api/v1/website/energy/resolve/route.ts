...
export async function POST(req: Request) {
  ...
  const streetNumber = text(body?.street_number ?? body?.streetNumber, 40)
  const apartment = text(body?.apartment, 40)
  const country = text(body?.country, 2)?.toUpperCase() ?? 'SE'
  const gridAreaCode = text(body?.grid_area_code ?? body?.gridAreaCode, 80)
  const facilityId = text(body?.facility_id ?? body?.facilityId, 120)
  const meteringPointId = text(body?.metering_point_id ?? body?.meteringPointId, 120)
  const requestedStartMode = text(body?.requested_start_mode ?? body?.requestedStartMode, 40)
  const requestedStartDate = text(body?.requested_start_date ?? body?.requestedStartDate, 10)
