...
type Props = {
  ...
  validatePricingQuote?: (input: { offerReference: string; priceAreaCode: string; estimatedMonthlyKwh: number; postalCode: string; city: string; address: string; quoteToken: string }) => Promise<{ ok: boolean; error?: string }>
}
...
function formatMaybeSekMonth(value: number | null | undefined) { return hasNumberValue(value) ? formatSekMonth(value) : 'Visas i prisofferten' }
function formatMaybeSekInvoice(value: number | null | undefined) { return hasNumberValue(value) ? formatSekInvoice(value) : 'Visas i prisofferten' }
function formatMaybeOre(value: number | null | undefined) { return hasNumberValue(value) ? formatOreKwh(value) : 'Visas i prisofferten' }
...
function SubmitButton({ disabled }: { disabled: boolean }) {
  ...
  return <button disabled={disabled || pending} className="h-12 rounded-2xl bg-cyan-500 px-8 font-bold text-black transition hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-cyan-300/70">{pending ? 'Skickar ansökan...' : 'Skicka ansökan'}</button>
}
...
export default function CustomerApplicationForm({ contracts, selectedValue, onSelectedValueChange, canSubmit, utm, action, energyResolution, pricingPreview, estimatedMonthlyKwh, contractDisplay, quoteContext, validatePricingQuote }: Props) {
  ...
  async function validateCurrentStep(): Promise<boolean> {
    ...
    if (step === 0) {
      ...
      if (!hasQuote) nextErrors.pricing = 'Räkna priset ovan innan du går vidare. Prisofferten måste gälla valt avtal, adress, elområde och förbrukning.'
    }
    if (step === 1) {
      ...
      if (!hasQuote || !pricingPreview?.quote_token || !selectedContract || !pricingPreview.kwh) nextErrors.pricing = 'Din prisoffert saknas eller har gått ut. Räkna om priset innan du går vidare.'
      if (Object.keys(nextErrors).length === 0 && validatePricingQuote && pricingPreview?.quote_token && selectedContract) {
        setCheckingQuote(true)
        const checked = await validatePricingQuote({ offerReference: selectedContract.offerReference, priceAreaCode: pricingPreview.price_area_code ?? pricingPreview.priceArea, estimatedMonthlyKwh: pricingPreview.kwh, postalCode: normalizePostalCode(form.postal_code), city: form.city.trim(), address: form.address.trim(), quoteToken: pricingPreview.quote_token })
        setCheckingQuote(false)
        if (!checked.ok) nextErrors.pricing = checked.error || 'Prisofferten stämmer inte längre med adressen eller förbrukningen. Räkna om priset.'
      }
    }
    if (step === 2) {
      if (!hasQuote) nextErrors.pricing = 'Prisofferten har gått ut. Räkna om priset innan du skickar ansökan.'
  ...
  return <div className="space-y-8" aria-live="polite">
    <ol className="grid gap-3 md:grid-cols-3" aria-label="Ansökningssteg">{STEPS.map((label, index) => <li key={label} className={`rounded-2xl border p-4 text-sm ${index === step ? 'border-cyan-500/50 bg-cyan-500/10 text-cyan-100' : index < step ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-100' : 'border-white/10 bg-white/5 text-gray-400'}`}><span className="font-semibold">Steg {index + 1}</span><span className="mt-1 block">{label}</span></li>)}</ol>
