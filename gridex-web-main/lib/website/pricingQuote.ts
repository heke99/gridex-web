...
import type { WebsitePricingPreview, WebsitePriceArea } from '@/lib/website/publicApi'
...
export function pricingLocationFingerprint(input: {
  ...
  const payload = [normalized(input.postalCode).replace(/\s/g, ''), normalized(input.city), normalized(input.address)].join('|')
...
function cloneBasis(value: unknown): QuoteBasis {
  ...
  if (
    ...
    return {
      ...
      ...(source === 'gridex_monthly_spot_prices' || source === 'gridex_spot_monthly_avg' || source === 'elprisetjustnu_api'
        ? { source }
        : {}),
    }
  }
  ...
  if (type === 'mix') {
    ...
    const source = typeof record.source === 'string' ? record.source.slice(0, 120) : undefined
    return {
      type,
...
function cloneFees(value: unknown): QuoteFees | undefined {
  ...
  for (const key of ['markupOre', 'variableFeeOre', 'elcertOre', 'monthlyFeeSek', 'invoiceFeeSek', 'billingIntervalMonths'] as const) {
    const parsed = number(source[key])
    if (parsed !== null) fees[key] = parsed
  }
  if (typeof source.invoiceFeeIncludedInMonthlyEstimate === 'boolean') {
    fees.invoiceFeeIncludedInMonthlyEstimate = source.invoiceFeeIncludedInMonthlyEstimate
  }
...
export function issueWebsitePricingQuote(input: {
  ...
  const totalMonthlyCostInclVatSek = number(input.preview.totalMonthlyCostInclVatSek)
  const pricePerKwhOre = number(input.preview.pricePerKwhOre)

  if (!secret || !locationFingerprint || !validArea(area) || totalMonthlyCostSek === null || totalMonthlyCostInclVatSek === null || pricePerKwhOre === null) {
    return null
  }
...
export function verifyWebsitePricingQuote(token: string | null | undefined, now = new Date()): PricingQuoteVerification {
  ...
  if (version !== QUOTE_VERSION || !payload || !signature || rest.length > 0 || !equalSignature(hmac(`${version}.${payload}`, secret), signature)) {
...
export function validateWebsitePricingQuote(input: {
  ...
  if (!fingerprint || !equalSignature(quote.location_fingerprint, fingerprint)) {
