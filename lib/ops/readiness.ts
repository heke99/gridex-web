...
export type OpsReadinessCode =
  | 'ready' | 'not_configured' | 'invalid_api_key' | 'missing_scope'
  | 'invalid_base_url_or_environment' | 'webhook_not_configured' | 'ops_unavailable'
...
export type OpsIntegrationReadiness = {
  ...
  webhook: { ready: boolean; enabled: boolean; signingSecretConfigured: boolean; expectedTenantReferenceConfigured: boolean; secretConflict: boolean }
}

type ProbeDefinition = { name: string; scopes: readonly string[]; run: () => Promise<{ ok: boolean; status: number; code: string | null } | void> }
...
function message(code: OpsReadinessCode): string {
  switch (code) {
    case 'ready': return 'OPS checkout-flöde, juridik och endpoint-behörigheter har verifierats.'
    ...
    case 'missing_scope': return 'API-nyckeln saknar minst en obligatorisk checkout-behörighet.'
    case 'invalid_base_url_or_environment': return 'OPS URL pekar på fel miljö eller saknar en dokumenterad endpoint.'
    case 'webhook_not_configured': return 'OPS API svarar, men webhooks är inte komplett konfigurerade.'
...
function authorizationProbe(path: string, method: 'GET' | 'POST', body?: Record<string, unknown>) {
  ...
  return probeOpsEndpointAuthorization(path, { method, headers, ...(body ? { body: JSON.stringify(body) } : {}) })
}
function probeDefinitions(): ProbeDefinition[] {
  return [
    ...
    { name: 'website_legal.read_any_of', scopes: GRIDEX_WEBSITE_LEGAL_SCOPE_ALTERNATIVES, run: () => authorizationProbe('/api/v1/website/legal-bundle', 'GET') },
  ]
}
...
export async function checkOpsIntegrationReadiness(): Promise<OpsIntegrationReadiness> {
  ...
  const scopeStatuses = new Map<string, ScopeStatus>()
  for (const scope of allScopes) scopeStatuses.set(scope, declared ? (declared.has(scope) ? 'declared' : 'missing') : 'unverified')
  ...
  const webhook = {
    ...
    secretConflict: Boolean(canonicalWebhookSecret && legacyWebhookSecret && canonicalWebhookSecret !== legacyWebhookSecret),
    ready: false,
  }
  webhook.ready = webhook.enabled && webhook.signingSecretConfigured && webhook.expectedTenantReferenceConfigured && !webhook.secretConflict
  const initialScopes = () => allScopes.map((scope) => ({ scope, status: scopeStatuses.get(scope) ?? 'unverified' }))
  if (!client.configured) return { ready: false, code: 'not_configured', message: message('not_configured'), checkedAt, probes: [], scopes: initialScopes(), webhook }
  ...
  for (let index = 0; index < results.length; index += 1) {
    ...
    if (settled.status === 'fulfilled') {
      ...
      for (const scope of definition.scopes) {
        if (ok) scopeStatuses.set(scope, declared?.has(scope) ? 'verified' : 'verified_not_declared')
      }
      ...
      probes.push({ name: definition.name, ok: false, status: isOpsError(settled.reason) ? settled.reason.status : null, code: errorCode(settled.reason) })
    }
  }

  const declarationComplete = !declared || (
    REQUIRED_WEBSITE_SCOPES.every((scope) => declared.has(scope)) &&
    GRIDEX_WEBSITE_LEGAL_SCOPE_ALTERNATIVES.some((scope) => declared.has(scope))
  )
  const legalProbe = probes.find((probe) => probe.name === 'website_legal.read_any_of')
  if (code === 'ready' && (!declarationComplete || !legalProbe?.ok || REQUIRED_WEBSITE_SCOPES.some((scope) => {
    const status = scopeStatuses.get(scope)
    return status === 'missing' || status === 'unverified'
  }))) code = 'missing_scope'
  const ready = code === 'ready' && probes.every((probe) => probe.ok)
  return { ready, code, message: message(code), checkedAt, probes, scopes: initialScopes(), webhook }
}
