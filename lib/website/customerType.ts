...
function normalizeContractCustomerType(value: string): WebsiteCustomerType | null {
...
/** Strict website boundary. `company` is the only documented legacy alias. */
...
export function contractSupportsCustomerType(customerTypes: string[] | null | undefined, customerType: WebsiteCustomerType): boolean {
  ...
  const supported = customerTypes.map(normalizeContractCustomerType).filter((value): value is WebsiteCustomerType => value !== null)
...
export function customerTypeLabel(customerTypes: string[] | null | undefined): string | null {
  ...
  const supported = new Set(customerTypes.map(normalizeContractCustomerType).filter((value): value is WebsiteCustomerType => value !== null))
