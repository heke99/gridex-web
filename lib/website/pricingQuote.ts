...
function publicQuoteFees(value: unknown): QuoteFees | undefined { return cloneRecord(value) as QuoteFees | undefined; }
...
function optionalLegacyDate(value: unknown): boolean { return value == null || validDate(value); }
...
function isQuote(value: unknown): value is WebsitePricingQuote {
  ...
  return q.version === 6 && validDate(createdAt) && validDate(q.issued_at) &&
    ...
    text(q.price_option_reference) && (q.area_price_reference === null || text(q.area_price_reference)) &&
    validInvoiceDeliveryMethod(q.invoice_delivery_method) &&
    validReferences(q.selected_component_references) && validReferences(q.mandatory_component_references) &&
...
export function issueWebsitePricingQuote(input: {
  ...
  const quote: WebsitePricingQuote = {
    ...
    contract: {
      ...
      contract_reference: input.preview.contract.contract_reference ?? null,
      product_code: input.contract.product_code ?? null,
    ...
    area_price_reference: input.preview.area_price_reference,
    ...
    pricing_interval: input.preview.pricing_interval as string,
    estimate_method: input.preview.estimate_method as string,
    ...
    pricing_snapshot_schema_version: input.preview.pricing_snapshot_schema_version as string,
  ...
  const unsigned = `${CURRENT_TOKEN_VERSION}.${keyring!.active.kid}.${encoded}`;
...
export function verifyWebsitePricingQuote(token: string | null | undefined, now = new Date()): PricingQuoteVerification {
  ...
  if (!ACCEPTED_TOKEN_VERSIONS.has(version) || !kid || !payload || !signature || rest.length || !secret ||
      !equalSignature(hmac(`${version}.${kid}.${payload}`, secret), signature)) return { ok: false, reason: "invalid" };
