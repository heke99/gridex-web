...
create index if not exists ops_webhook_events_related_entity_idx
  on public.ops_webhook_events(related_entity_type, related_entity_id, received_at desc)
  where related_entity_id is not null;

create or replace function public.apply_ops_domain_event(
  p_event_id text,
  p_delivery_id text,
  p_event_type text,
...
begin
  ...
  then
    raise exception 'Canonical domain webhook arguments are incomplete.';
  end if;
  ...
  if coalesce(cardinality(v_user_ids), 0) > 1 then
    if v_event_row_id is null then
      insert into public.ops_webhook_events (
        event_id, event_type, delivery_id, tenant_reference, customer_id,
        ...
        p_event_id, p_event_type, p_delivery_id, p_tenant_reference, p_customer_id,
  ...
  if v_event_row_id is null then
    insert into public.ops_webhook_events (
      ...
      tenant_reference, customer_id, customer_number, external_customer_id,
      ...
      p_tenant_reference, p_customer_id, p_customer_number, p_external_customer_id,
      ...
      'signature_and_tenant_verified', 1, 5, now()
  ...
  begin
    ...
    if nullif(trim(p_notification_title), '') is not null and nullif(trim(p_notification_body), '') is not null then
      insert into public.customer_notifications (
        ...
        jsonb_build_object('source', 'ops_webhook', 'event_type', p_event_type, 'tenant_reference', p_tenant_reference),
...
revoke all on function public.apply_ops_domain_event(
  text, text, text, text, timestamptz, text, jsonb,
  text, text, text, text, text, text, text, text, text, text, text
) from public, anon, authenticated;
grant execute on function public.apply_ops_domain_event(
  text, text, text, text, timestamptz, text, jsonb,
  text, text, text, text, text, text, text, text, text, text, text
) to service_role;

-- Correct the legacy behavior that marked unknown events as business-processed.
-- Unknown signed events are retained and explicitly classified as ignored.
...
begin
  ...
  select * into v_existing
    from public.ops_webhook_events
   where event_id = p_event_id or delivery_id = p_delivery_id
   order by received_at asc
   limit 1
   for update;
  ...
  insert into public.ops_webhook_events (
    event_id, event_type, delivery_id, header_event_id, header_event_type,
    tenant_reference, occurred_at, status, signature_valid, payload_hash,
    payload, handling_note, processed_at
  ) values (
    p_event_id, p_event_type, p_delivery_id, p_event_id, p_event_type,
    p_tenant_reference, p_created_at, 'ignored', true, p_payload_hash,
    p_payload, 'ignored_unknown_type', now()
  );

  return query select 'ignored'::text, false, null::bigint;
end;
$$;
