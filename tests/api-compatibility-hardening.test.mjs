...
assert.match(client, /ops_customer_application_portal_identity_contract_unsupported/, 'signup must fail closed if a future OPS schema drops the identity fields')
assert.match(client, /websiteSchemaHasProperty\('CustomerApplicationRequest', 'customer_portal_user_id'\)/, 'signup must require the portal identity field to remain published in OpenAPI')
assert.match(client, /websiteSchemaHasProperty\('CustomerApplicationRequest', 'auth_user_id'\)/, 'signup must require the auth identity field to remain published in OpenAPI')
assert.match(client, /if \(hasCustomerPortalUserId !== hasAuthUserId\)/, 'portal identity must be supplied as a complete pair or omitted entirely')
assert.match(client, /uuidPattern\.test\(customerPortalUserId\).*uuidPattern\.test\(authUserId\)/s, 'supplied identity fields must be valid UUID values')
assert.match(client, /if \(customerPortalUserId !== authUserId\)/, 'supplied identity fields must be equal')
assert.match(client, /customerPortalUserId && authUserId[\s\S]*customer_portal_user_id:\s*customerPortalUserId[\s\S]*auth_user_id:\s*authUserId/s, 'anonymous signup must omit the optional identity pair while authenticated signup sends both')
