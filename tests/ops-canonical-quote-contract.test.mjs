...
assert.ok(preview.includes("usesElprisetJustNu(model) || model === 'mix'"))
assert.ok(preview.includes('buildLocalWebsitePricingPreview'))
assert.ok(preview.includes('return fetchOpsWebsiteQuote(input)'))
