...
const energyResolutionSchema = websiteOpenApi.components.schemas.EnergyAreaResolution
...
assert.ok(area.includes('resolution.automation_allowed !== true'))
assert.ok(area.includes("code: 'resolution_not_ready'"))
assert.ok(ops.includes("automation_allowed: pickBoolean(row, ['automation_allowed', 'automationAllowed'])"))
assert.ok(ops.includes("next_required_action: pickString(row, ['next_required_action', 'nextRequiredAction'])"))
assert.ok(energyToken.includes("const TOKEN_VERSION = 'ea3'"))
assert.ok(energyToken.includes('input.resolution.automation_allowed !== true'))
...
assert.ok(energyResolutionSchema.required.includes('resolution_status'))
assert.ok(energyResolutionSchema.required.includes('automation_allowed'))
assert.ok(energyResolutionSchema.required.includes('warnings'))
assert.equal(energyResolutionSchema.properties.automation_allowed.type, 'boolean')
assert.ok(!('price_area_code' in energyResolutionSchema.properties))
assert.ok('price_area' in energyResolutionSchema.properties)
assert.ok('expires_at' in energyResolutionSchema.properties)
