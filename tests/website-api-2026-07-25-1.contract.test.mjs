...
assert.ok(contract.includes("GRIDEX_WEBSITE_API_CONTRACT_VERSION = '2026-07-25.1'"))
...
assert.ok(energyToken.includes("const TOKEN_VERSION = 'ea4'"))
...
assert.equal(websiteOpenApi.info.version, '2026-07-25.1')

assert.equal(customerPortalOpenApi.info.version, '2026-07-25.1')
...
for (const scope of [
  ...
  customerPortalOpenApi.paths['/api/v1/customer/portal-bundle'].get['x-required-scopes'].includes(scope),
...
assert.ok(ops.includes('await opsCustomerFetch("/api/v1/customer/portal-bundle", identity, {\n        method: "GET"'))
...
console.log('Website API 2026-07-25.1 contract tests passed')
